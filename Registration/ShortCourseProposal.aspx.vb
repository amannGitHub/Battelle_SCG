...
Imports System.IO
Public Class ShortCourseProposal
    ...
    Protected Sub ButtonSubmitAbstract_Click(sender As Object, e As EventArgs) Handles ButtonSubmitAbstract.Click
        ...
        If rblCourseLength.SelectedValue = "" Then
            IsValidAbstract = False
            ValidationFailures.Add("Course Length", "Please indicate course length.")
        End If
        If rblLaptopsYN.SelectedValue = "" Then
            IsValidAbstract = False
            ValidationFailures.Add("Laptops", "Please indicate if laptops are required.")
        End If
        If rblSoftwareYN.SelectedValue = "" Then
            IsValidAbstract = False
            ValidationFailures.Add("Specialty Software", "Please indicate if you require specialty software.")
        End If
        If rblSoftwareYN.SelectedValue = "True" And txtSoftware.Text.Trim() = "" Then
            IsValidAbstract = False
            ValidationFailures.Add("Specialty Software Needed", "Please list the required software and explain how it will be distributed to attendees.")
        End If

        If rblInternetYN.SelectedValue = "" Then
            IsValidAbstract = False
            ValidationFailures.Add("Internet Access", "Please indicate if internet access is required for the course.")
        End If

        If rblSupplementaryMaterialsYN.SelectedValue = "" Then
            IsValidAbstract = False
            ValidationFailures.Add("Specialty Software", "Please indicate if you require specialty software.")
        End If
        If rblSupplementaryMaterialsYN.SelectedValue = "True" And txtSupplementaryMaterials.Text.Trim() = "" Then
            IsValidAbstract = False
            ValidationFailures.Add("Specialty Software Needed", "Please list the supplementary materials.")
        End If
        ...
        End If
        If String.IsNullOrWhiteSpace(txtOverview.Text) Then
            IsValidAbstract = False
            ValidationFailures.Add("Description", "Please provide a Description.")
        End If
        If String.IsNullOrWhiteSpace(txtAgenda.Text) Then
            IsValidAbstract = False
            ValidationFailures.Add("Draft Agenda", "Please provide a Draft Agenda.")
        End If
        ...
        If IsValidAbstract Then
            ...
            Dim InsertShortCourseCommand As SqlCommand = New SqlCommand("ShortCourseProposalInsert")

            ' Set rest of parameters using form data
            InsertShortCourseCommand.Parameters.AddWithValue("PersonID", PersonID)
            InsertShortCourseCommand.Parameters.AddWithValue("CourseName", txtTitle.Text.Trim())
            InsertShortCourseCommand.Parameters.AddWithValue("CourseLength", rblCourseLength.SelectedValue)
            InsertShortCourseCommand.Parameters.AddWithValue("LaptopsRequired", rblLaptopsYN.SelectedValue)
            InsertShortCourseCommand.Parameters.AddWithValue("SpecialSoftwareRequired", rblSoftwareYN.SelectedValue)
            InsertShortCourseCommand.Parameters.AddWithValue("Software", txtSoftware.Text.Trim())
            InsertShortCourseCommand.Parameters.AddWithValue("InternetAccessRequired", rblInternetYN.SelectedValue)
            InsertShortCourseCommand.Parameters.AddWithValue("SupplementalMaterialsProvided", rblSupplementaryMaterialsYN.SelectedValue)
            InsertShortCourseCommand.Parameters.AddWithValue("SupplementalMaterialsList", txtSupplementaryMaterials.Text.Trim())
            InsertShortCourseCommand.Parameters.AddWithValue("CourseObjective", txtObjective.Text.Trim())
            InsertShortCourseCommand.Parameters.AddWithValue("CourseOverview", txtOverview.Text.Trim())
            InsertShortCourseCommand.Parameters.AddWithValue("DraftAgenda", txtAgenda.Text.Trim())
            ' conf id

            InsertShortCourseCommand.Parameters.AddWithValue("ConferenceID", ConferenceControl.ConferenceID)

            Dim DSReturnData As DataSet
            Dim PresenterPersonID As Integer
            Dim InstructorList As String = ""

            DSReturnData = BattelleExecuteCommand(InsertShortCourseCommand)
            If DSReturnData.Tables(0).Rows.Count > 0 Then
                ...
                For Each LI As ListItem In ListBoxAbstractInstructors.Items
                    ...
                    End If
                    BattelleShortCourseInstructorsInsert(AbstractID, Integer.Parse(LI.Value), POCInstructor)
            ...
            SubmittedConfirmationHTML += "Course Title:<br /><b>" + txtTitle.Text.Trim() + "</b><br /><br />"
            SubmittedConfirmationHTML += "Instructors:<br /><b>" + InstructorList + "</b><br /><br />"
            SubmittedConfirmationHTML += "Course Length<br /><b>" + rblCourseLength.SelectedItem.Text + "</b><br /><br />"
            SubmittedConfirmationHTML += "Will participants need to bring laptops for classroom exercises?<br /><b>" + rblLaptopsYN.SelectedItem.Text + "</b><br /><br />"
            SubmittedConfirmationHTML += "Will participants be required to download and use any specialty software?<br /><b>" + rblSoftwareYN.SelectedItem.Text + "</b><br /><br />"
            If rblSoftwareYN.SelectedValue = "True" Then
                SubmittedConfirmationHTML += "Required software and how it will be distributed to attendees:<br /><b>" + txtSoftware.Text + "</b><br /><br />"
            End If
            SubmittedConfirmationHTML += "Will Internet access be required for the course Instructor or attendees’ use during the course?<br /><b>" + rblInternetYN.SelectedItem.Text + "</b><br /><br />"
            SubmittedConfirmationHTML += "In addition to the course handbook, will supplementary printed or digital materials be provided to participants?<br /><b>" + rblSupplementaryMaterialsYN.SelectedItem.Text + "</b><br /><br />"
            If rblSupplementaryMaterialsYN.SelectedValue = "True" Then
                SubmittedConfirmationHTML += "Supplementary Materials:<br /><b>" + txtSupplementaryMaterials.Text + "</b><br /><br />"
            End If
            SubmittedConfirmationHTML += "Course Objective<br /><b>" + txtObjective.Text + "</b><br /><br />"
            SubmittedConfirmationHTML += "Course Overview<br /><b>" + txtOverview.Text + "</b><br /><br />"
            SubmittedConfirmationHTML += "Draft Agenda<br /><b>" + txtAgenda.Text + "</b><br /><br />"
            SubmittedConfirmationHTML += "</p>"

            SubmittedConfirmationHTML += "<br /><br />"


            Dim AbstractSubmissionEmail As DataRow = BattelleEmailGetByNameAsDataRow(ConferenceControl.ConferenceURLString + "_shortcourseproposal_confirmation")
            Dim AbstractSubmissionEmailHTML As String
            If AbstractSubmissionEmail Is Nothing Then
                AbstractSubmissionEmail = BattelleEmailGetByNameAsDataRow("shortcourseproposal_confirmation")
            End If
            ...
            AbstractSubmissionEmailHTML = AbstractSubmissionEmailHTML.Replace("{[shortcourse_confirmation_content]}", SubmittedConfirmationHTML)



            'If abstract has a presenting Instructor who is different than user who submitted, send Short Course email confirmation to both'
            If PresenterPersonID > 0 And PresenterPersonID <> PersonID Then
                ...
                BattelleSendMail(AbstractSubmissionEmailHTML, PersonData("Email"), "Short Course Submission for " + ConferenceControl.ConferenceShortName, Net.Mail.MailPriority.Normal, ConferenceControl.ConferenceShortName, ConferenceControl.ConferencePlannerEmail, "phippss@battelle.org,melaragm@battelle.org")
                BattelleSendMail(AbstractSubmissionEmailHTML, PresenterData("Email"), "Short Course Submission for " + ConferenceControl.ConferenceShortName, Net.Mail.MailPriority.Normal, ConferenceControl.ConferenceShortName, ConferenceControl.ConferencePlannerEmail)
            Else
                AbstractSubmissionEmailHTML = AbstractSubmissionEmailHTML.Replace("{[first_name]}", PersonData("FirstName"))
                AbstractSubmissionEmailHTML = AbstractSubmissionEmailHTML.Replace("{[last_name]}", PersonData("LastName"))
                BattelleSendMail(AbstractSubmissionEmailHTML, PersonData("Email"), "Short Course Submission for " + ConferenceControl.ConferenceShortName, Net.Mail.MailPriority.Normal, ConferenceControl.ConferenceShortName, ConferenceControl.ConferencePlannerEmail, "phippss@battelle.org,melaragm@battelle.org")
    ...
    Protected Sub ResetForm()
        ...
        PanelForm.Visible = True
        txtTitle.Text = ""
        rblCourseLength.ClearSelection()
        rblLaptopsYN.ClearSelection()
        rblSoftwareYN.ClearSelection()
        txtSoftware.Text = ""
        rblInternetYN.ClearSelection()
        rblSupplementaryMaterialsYN.ClearSelection()
        txtSupplementaryMaterials.Text = ""
        txtObjective.Text = ""
        txtOverview.Text = ""
        txtAgenda.Text = ""
