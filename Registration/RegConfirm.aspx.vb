...
Public Class RegConfirm
            ...
            If Session("PersonID") IsNot Nothing AndAlso Session("PersonID").ToString <> "" Then
                If Context.GetOwinContext().Authentication.User.Identity.Name = "SCGadmin" Then
                    If Page.RouteData.Values("admin") = "admin" Then
                        ...
                        hdnNewStart.Value = "No" 'Send email
                    Else
                        hdnNewStart.Value = "Yes" 'No email
                    End If
                Else
                    hdnNewStart.Value = "No" 'Send email
                End If
    ...
    Protected Sub FormViewConfirmation_DataBound(sender As Object, e As EventArgs) Handles FormViewConfirmation.DataBound
        ...
        RegistrationConfirmationHTML += "<b>Name:</b> " + PersonData("FirstName").ToString + " " + PersonData("LastName").ToString + "<br />"
        RegistrationConfirmationHTML += "<b>Participant Code:</b> <i>" + PersonData("ParticipationID").ToString + "</i><br />"
        RegistrationConfirmationHTML += "<b>Address Line 1:</b> " + PersonData("AddressLine1").ToString + "<br />"
        RegistrationConfirmationHTML += "<b>Address Line 2:</b> " + PersonData("AddressLine2").ToString + "<br />"
        RegistrationConfirmationHTML += "<b>Address Line 3:</b> " + PersonData("AddressLine3").ToString + "<br />"
        RegistrationConfirmationHTML += "<b>City:</b> " + PersonData("City").ToString + "<br />"
        RegistrationConfirmationHTML += "<b>State/Province:</b> " + PersonData("StateProvince").ToString + "<br />"
        RegistrationConfirmationHTML += "<b>Zip/Postal Code:</b> " + PersonData("ZipPostal").ToString + "<br />"
        RegistrationConfirmationHTML += "<b>Country:</b> " + PersonData("Country").ToString
        RegistrationConfirmationHTML += "</p>"


        Dim BoothReservationEmail As DataRow = BattelleEmailGetByNameAsDataRow(ConferenceControl.ConferenceURLString + "_registration_confirmation")
        Dim RegistrationEmailHTML As String
        If BoothReservationEmail Is Nothing Then
            BoothReservationEmail = BattelleEmailGetByNameAsDataRow("registration_confirmation")
        End If
    ...
    Protected Sub Page_PreRenderComplete(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.PreRenderComplete
        If hdnNewStart.Value = "No" Then
            ...
            BattelleSendMail(RegistrationEmailHTML, PersonData("Email"), ConferenceControl.ConferenceShortName + " Registration Confirmation", Net.Mail.MailPriority.Normal, ConferenceControl.ConferenceShortName, ConferenceControl.ConferencePlannerEmail)
    ...
    Protected Sub FormViewReg_DataBound(sender As Object, e As EventArgs) Handles FormViewReg.DataBound
        ...
        If DataRow("OneDayPassDate").ToString <> "" Then
            Dim sOneDay As String = DataRow("OneDayPassDate") 'No longer a Date type

            RegEmail += "<b>Selected Attendance Date(s):</b> " + sOneDay + "<br />"
        End If
