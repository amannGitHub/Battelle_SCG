...
Public Class LearningLabManager
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        If IsPostBack Then
            SqlDataSourceLearningLab.DataBind()
            SqlDataSourceLearningLabExport.DataBind()
    ...
    Protected Sub SqlDataSourceLearningLab_Updating(sender As Object, e As SqlDataSourceCommandEventArgs) Handles SqlDataSourceLearningLab.Updating
        SqlDataSourceLearningLab.UpdateParameters("LearningLabID").DefaultValue = FormView1.DataKey.ToString
        SqlDataSourceLearningLabExport.UpdateParameters("LearningLabID").DefaultValue = FormView1.DataKey.ToString

    End Sub


    Protected Sub gvLab_SelectedIndexChanged(sender As Object, e As EventArgs) Handles gvLab.SelectedIndexChanged
        FormView1.Visible = True
        gvAuthors.Visible = True
        LearningLabHeader.Visible = True
    ...
    Protected Sub ddlConfList_SelectedIndexChanged(sender As Object, e As EventArgs) Handles ddlConfList.SelectedIndexChanged
        gvLab.Visible = True 'Code reused from Abstract manager, keeping the name for simplicity
        gvAuthorsButton.Visible = False
        HdnConferenceID.Value = ddlConfList.SelectedValue
        LearningLabHeader.Visible = False
        gvCourseAuthorsLbl.Visible = False
    ...
    Protected Sub SqlDataSourceLabSelected_Selecting(sender As Object, e As SqlDataSourceSelectingEventArgs) Handles SqlDataSourceLabSelected.Selecting

    End Sub

    Protected Sub SqlDataSourceLabSelected_Selected(sender As Object, e As SqlDataSourceStatusEventArgs) Handles SqlDataSourceLabSelected.Selected
    ...
    Protected Sub SqlDataSourceLabSelected_Updated(sender As Object, e As SqlDataSourceStatusEventArgs) Handles SqlDataSourceLabSelected.Updated
        'SqlDataSourceShortCourse.SelectParameters("ConferenceID").DefaultValue = FormView1.DataKey.ToString
        SqlDataSourceLearningLab.SelectParameters("ConferenceID").DefaultValue = HdnConferenceID.Value
        SqlDataSourceLearningLab.DataBind()
        gvLab.DataBind()

        SqlDataSourceLearningLabExport.SelectParameters("ConferenceID").DefaultValue = HdnConferenceID.Value
        SqlDataSourceLearningLabExport.DataBind()
        gvLearningLabAll.DataBind()
    ...
    Protected Sub AbstractAuthorsUpdate_Click()
        ...
        Dim LearningLabID As Integer = FormView1.DataKey.Value
        ...
        For Each LA As ListItem In listofDeletedAuthors.Items
            BattelleLearningLabInstructorsDelete(LearningLabID, Integer.Parse(LA.Value))
        Next
        ...
        For Each LI As ListItem In ListBoxAbstractAuthors.Items
            ...
            BattelleLearningLabInstructorsUpdate(LearningLabID, Integer.Parse(LI.Value), Presenter)
        Next
    ...
    Protected Sub ButtonLearningLabExport_Click(sender As Object, e As EventArgs) Handles ButtonLearningLabExport.Click

        Dim regDate As Date = Date.Now()
        Dim strDate As String = regDate.ToString("ddMMMyyyy")
        Dim fileName As String = "LearningLabManager_Export" + strDate + ".xls"

        'activate the full data GridView
        gvLearningLabAll.Visible = True


        ' Let's hide all unwanted stuffing
        gvLearningLabAll.AllowPaging = False
        gvLearningLabAll.AllowSorting = False
        ...
        Me.gvLearningLabAll.Parent.Controls.Add(frm)
        frm.Attributes("runat") = "server"
        frm.Controls.Add(Me.gvLearningLabAll)
        frm.RenderControl(hwriter)

        Response.Write(swriter.ToString())
        Response.[End]()

        gvLearningLabAll.Visible = False
